...
| `findOne(options?)` | `Promise<T \| null>` | Sugar: builds a `QueryBuilder<T>`, applies options, calls `getOne()`. |
| `findMany(options?)` | `Promise<T[]>` | Sugar: same shape as `findOne`, calls `getMany()`. |
| `find()` | `QueryBuilder<T>` | **Handoff.** Returns the builder; runs no SQL. |
| `update(entity: T)` | `Promise<...>` | Overwrite a row identified by its primary key. |
| `delete(key)` | `Promise<...>` | Remove a row by its primary key. |

`find()` and the sugar wrappers are the read surface. `create`, `update`, `delete` are the write surface. `findById` is the only by-key reader; everything else read-side flows through the builder.
...
`find()` does not run SQL. It returns a `QueryBuilder<T>` and steps out. Everything about clause accumulation, lazy execution, the `where` callback, and SQL composition lives in [[QueryBuilder]] / [[Lazy Query Builder]].

`findOne(options?)` and `findMany(options?)` are *sugar* — they construct a builder, apply `FindOptions`, and call the terminal method for you. Use them when the call site doesn't need to compose; reach for `find()` when it does.
...
- [[QueryBuilder]] — what `find()` returns; what `findOne`/`findMany` use internally.
